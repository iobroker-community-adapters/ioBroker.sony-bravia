...
class Bravia {
    ...
    getContentList(_startIndex, _count, _source) {
        return new Promise((resolve, reject) => {
            this._jsonRequest("avContent", "getContentList", [{ stIdx: _startIndex, cnt: _count, uri: _source }], "1.5").then(body => {
                if (body.result) {
                    resolve(body.result[0]);
                } else {
                    let err = JSON.stringify(body);
                    reject(new Error(`getContentList. Response error. Missing result. ${err}`));
                }
            }).catch(error => {
                reject(error);
            });
