...
class Bravia {
                    ...
                    if (/^[A]{5}[a-zA-Z0-9]{13}[\=]{2}$/.test(code)) {
                        ...
                        this.getIRCCCodes()
                            .then(response => {
                                ...
                                send(ircc.value).then(resolve, reject);
                            }, reject);
                    }
                } else {
                    resolve();
    ...
    /**
     * https://pro-bravia.sony.net/develop/integrate/rest-api/spec/service/audio/v1_0/getVolumeInformation/
     * {
     *   "result": [[
     *   {
     *       "volume": 25,
    ...
    getVolumeInformation() {
        return new Promise((resolve, reject) => {
            this._jsonRequest("audio", "getVolumeInformation").then(body => {
                if (body.result) {
                    resolve(body.result);
                }
                else if (body.error && body.error[0] == 40005) {
                    // Display Is Turned off
                    resolve(body.error);
                } else {
                    let err = JSON.stringify(body);
                    reject(new Error(`getVolumeInformation. Response error. Missing result. ${err}`));
                }
            }).catch(error => {
                reject(error);
            });
    ...
    setVolume(_target, _volume) {
        return new Promise((resolve, reject) => {
            this.getSupportedApiVersion("audio", "setAudioVolume").then(version => {
                this.setVolumeInternal(_target, _volume, version).then(result => {
                    resolve(result);
                }).catch(error => {
                    reject(error);
                });
            }).catch(error => {
                // error handling issue #92 - test user fixed version
                this.setVolumeInternal(_target, _volume, "1.0").then(result => {
                    resolve(result);
                }).catch(error => {
                    resolve([]);
                });
    ...
    setVolumeInternal(_target, _volume, _version) {
        return new Promise((resolve, reject) => {
            this._jsonRequest("audio", "setAudioVolume", [{ volume: _volume, target: _target }], _version).then(body => {
                if (body.result) {
                    resolve(body.result[0]);
                } else {
                    let err = JSON.stringify(body);
                    reject(new Error(`setAudioVolume ${_target} ${_version}. Response error. Missing result. ${err}`));
                }
            }).catch(error => {
                reject(error);
            });
    ...
    setMute(_status) {
        return new Promise((resolve, reject) => {
            this._jsonRequest("audio", "setAudioMute", [{ status: _status }], "1.0").then(body => {
                if (body.result) {
                    resolve(body.result[0]);
                } else {
                    let err = JSON.stringify(body);
                    reject(new Error(`setAudioMute. Response error. Missing result. ${err}`));
                }
            }).catch(error => {
                reject(error);
            });
    ...
    getContentList(_startIndex, _count, _source) {
        return new Promise((resolve, reject) => {
            this.getSupportedApiVersion("avContent", "getContentList").then(version => {
                this.getContentListInternal(_startIndex, _count, _source, version).then(result => {
                    resolve(result);
                }).catch(error => {
                    reject(error);
                });
                ...
                // error handling issue #92 - test user fixed version
                this.getContentListInternal(_startIndex, _count, _source, "1.0").then(result => {
                    resolve(result);
                }).catch(error => {
                    resolve([]);
                });
    ...
    getContentListInternal(_startIndex, _count, _source, _version) {
        return new Promise((resolve, reject) => {
            this._jsonRequest("avContent", "getContentList", [{ stIdx: _startIndex, cnt: _count, uri: _source }], _version).then(body => {
                if (body.result) {
                    resolve(body.result[0]);
                } else {
                    let err = JSON.stringify(body);
                    reject(new Error(`getContentList ${_version} ${_source}. Response error. Missing result. ${err}`));
                }
            }).catch(error => {
                reject(error);
            });
