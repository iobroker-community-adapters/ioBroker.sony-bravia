...
class Bravia {
    ...
    getContentListTvDvbs(startIndex, count) {
        return new Promise((resolve, reject) => {
            this._jsonRequest("avContent", "getContentList", [{ stIdx: startIndex, cnt: count, uri: "tv:dvbs" }], "1.5").then(body => {
                if (body.result) {
                    resolve(body.result[0]);
                } else {
                    let err = JSON.stringify(body);
                    reject(new Error(`getContentListTvDvbs. Response error. Missing result. ${err}`));
                }
            }).catch(error => {
                reject(error);
            });
    ...
    _jsonRequest(_serviceProtocol/*: string*/, _method/*: string*/, _params/*: string[]*/ = [], _version = "1.0", _id = 3) {
        return new Promise((resolve, reject) => {
            Request.post({
